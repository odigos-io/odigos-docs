...
![](/images/setup_sources.png)

After Odigos detected all the applications in the cluster, configure:

### Opt In / Opt Out

Use the `Apply for any future apps` for every namespace.

If the checkbox is **unchecked**, then new applications in the namespace will not be instrumented automatically.
To add them as sources, you will need to manually select them in the UI.
Opt in mode is recommended when you want to have greater control over which applications are instrumented.

When the checkbox is **checked**, then new applications in the namespace will be instrumented automatically.

### Application Instrumentation

From the applications list, you can manually select which applications you want to instrument.
